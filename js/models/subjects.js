...
export function buildClassPool(catalog, {year, maxClassSize, you}) {
  ...
  const pushClass = (subject, codePrefix, teacher, room=null) => {
    ...
    const candidateRooms = catalog.roomBinding[subject] || [];
    const studioRoom = candidateRooms.length
      ? candidateRooms[(codeCounter[codePrefix]-1) % candidateRooms.length]
      : null;
    const finalRoom = studioRoom || room || randomRoom();
    classes.push({ code, subject, teacher, room: finalRoom, year, max: maxClassSize });
  };

  // Maths/English sets (8 each gives capacity up to 176 with 22 cap; combined with others covers 180)
  ...
  // Electives (Drama + Drama Tech to you, with Studio rooms via roomBinding)
